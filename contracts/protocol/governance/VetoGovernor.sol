...
abstract contract VetoGovernor is Context, ERC165, EIP712, IVetoGovernor, IERC721Receiver, IERC1155Receiver {
  ...
  function propose(
    ...
    require(getVotes(proposer, tokenId, currentTimepoint - 1) >= proposalThreshold(), "Governor: proposer votes below proposal threshold");
  ...
  function _veto(uint256 proposalId) internal returns (uint256) {
    ...
    require(
      status != ProposalState.Vetoed && status != ProposalState.Canceled && status != ProposalState.Expired && status != ProposalState.Executed,
