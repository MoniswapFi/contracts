...
contract Minter is IMinter {
  ...
  function initialize(AirdropParams memory params) external {
    ...
    if ((params.liquidWallets.length != params.liquidAmounts.length) || (params.lockedWallets.length != params.lockedAmounts.length))
    ...
    for (uint256 i = 0; i < _len; i++) {
      moni.mint(params.liquidWallets[i], params.liquidAmounts[i]);
  ...
  function calculateGrowth(uint256 _minted) public view returns (uint256 _growth) {
    ...
    uint256 _aeroTotal = moni.totalSupply();
  ...
  function updatePeriod() external returns (uint256 _period) {
    ...
    if (block.timestamp >= _period + WEEK) {
      ...
      uint256 _emission;
      uint256 _totalSupply = moni.totalSupply();
      ...
      uint256 _balanceOf = moni.balanceOf(address(this));
      if (_balanceOf < _required) {
        moni.mint(address(this), _required - _balanceOf);
      }

      moni.safeTransfer(address(team), _teamEmissions);
      moni.safeTransfer(address(rewardsDistributor), _growth);
      rewardsDistributor.checkpointToken(); // checkpoint token balance that was just minted in rewards distributor

      moni.safeApprove(address(voter), _emission);
      voter.notifyRewardAmount(_emission);

      emit Mint(msg.sender, _emission, moni.totalSupply(), _tail);
    }
  }
}
