...
contract VotingEscrow is IVotingEscrow, ERC2771Context, ReentrancyGuard {
  ...
  function createManagedLockFor(address _to) external nonReentrant returns (uint256 _mTokenId) {
    ...
    (address _lockedManagedReward, address _freeManagedReward) = IManagedRewardsFactory(IFactoryRegistry(factoryRegistry).managedRewardsFactory())
  ...
  function withdrawManaged(uint256 _tokenId) external nonReentrant {
    ...
    newLockedManaged.amount -= (_total.toInt128() < newLockedManaged.amount ? _total.toInt128() : newLockedManaged.amount);
  ...
  function setManagedState(uint256 _mTokenId, bool _state) external {
    if (_msgSender() != IVoter(voter).emergencyCouncil() && _msgSender() != IVoter(voter).governor()) revert NotEmergencyCouncilOrGovernor();
    if (escrowType[_mTokenId] != EscrowType.MANAGED) revert NotManagedNFT();
  ...
  function _checkpoint(
    ...
    GlobalPoint memory lastPoint = GlobalPoint({bias: 0, slope: 0, ts: block.timestamp, blk: block.number, permanentLockBalance: 0});
    if (_epoch > 0) {
  ...
  function _depositFor(
    ...
    (newLocked.amount, newLocked.end, newLocked.isPermanent) = (_oldLocked.amount, _oldLocked.end, _oldLocked.isPermanent);
  ...
  function split(uint256 _from, uint256 _amount) external nonReentrant returns (uint256 _tokenId1, uint256 _tokenId2) {
    ...
    emit Split(_from, _tokenId1, _tokenId2, sender, _locked[_tokenId1].amount.toUint256(), _splitAmount.toUint256(), newLocked.end, block.timestamp);
  }
  ...
  bytes32 public constant DELEGATION_TYPEHASH = keccak256("Delegation(uint256 delegator,uint256 delegatee,uint256 nonce,uint256 expiry)");
  ...
  function _checkpointDelegator(
    ...
    DelegationLogicLibrary.checkpointDelegator(_locked, numCheckpoints, _checkpoints, _delegates, _delegator, _delegatee, _owner);
  }
