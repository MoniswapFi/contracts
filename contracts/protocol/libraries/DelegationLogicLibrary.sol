...
library DelegationLogicLibrary {
  ...
  function checkpointDelegator(
    ...
    IVotingEscrow.Checkpoint storage cpOld = numCheckpoint > 0 ? _checkpoints[_delegator][numCheckpoint - 1] : _checkpoints[_delegator][0];
  ...
  function checkpointDelegatee(
    ...
    IVotingEscrow.Checkpoint storage cpOld = numCheckpoint > 0 ? _checkpoints[_delegatee][numCheckpoint - 1] : _checkpoints[_delegatee][0];
    ...
    cp.delegatedBalance = _increase ? cpOld.delegatedBalance + balance_ : (balance_ < cpOld.delegatedBalance ? cpOld.delegatedBalance - balance_ : 0);
  ...
  function getPastVotes(
    ...
    return lastCheckpoint.delegatee == 0 ? votes + IVotingEscrow(address(this)).balanceOfNFTAt(_tokenId, _timestamp) : votes;
  }
}
