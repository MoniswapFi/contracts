...
abstract contract Reward is IReward, ERC2771Context, ReentrancyGuard {
  ...
  function _writeCheckpoint(uint256 tokenId, uint256 balance) internal {
    ...
    if (
      _nCheckPoints > 0 &&
      ProtocolTimeLibrary.epochStart(checkpoints[tokenId][_nCheckPoints - 1].timestamp) == ProtocolTimeLibrary.epochStart(_timestamp)
    ) {
  ...
  function _writeSupplyCheckpoint() internal {
    ...
    if (
      _nCheckPoints > 0 &&
      ProtocolTimeLibrary.epochStart(supplyCheckpoints[_nCheckPoints - 1].timestamp) == ProtocolTimeLibrary.epochStart(_timestamp)
    ) {
