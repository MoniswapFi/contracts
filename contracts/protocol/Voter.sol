...
contract Voter is IVoter, ERC2771Context, ReentrancyGuard {
  ...
  function vote(
    ...
    if ((_timestamp > ProtocolTimeLibrary.epochVoteEnd(_timestamp)) && !isWhitelistedNFT[_tokenId]) revert NotWhitelistedNFT();
  ...
  function createGauge(address _poolFactory, address _pool) external nonReentrant returns (address) {
    ...
    (address votingRewardsFactory, address gaugeFactory) = IFactoryRegistry(factoryRegistry).factoriesToPoolFactory(_poolFactory);
    ...
    (address _feeVotingReward, address _bribeVotingReward) = IVotingRewardsFactory(votingRewardsFactory).createRewards(forwarder, rewards);

    address _gauge = IGaugeFactory(gaugeFactory).createGauge(forwarder, _pool, _feeVotingReward, rewardToken, isPool);
    ...
    emit GaugeCreated(_poolFactory, votingRewardsFactory, gaugeFactory, _pool, _bribeVotingReward, _feeVotingReward, _gauge, sender);
    return _gauge;
  }
