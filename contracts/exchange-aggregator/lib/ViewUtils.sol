...
import {Offer, FormattedOffer} from "../interfaces/IAggregatorRouter.sol";
...
library OfferUtils {
  ...
  function format(Offer memory _queries) internal pure returns (FormattedOffer memory) {
    return FormattedOffer(_queries.amounts.toUints(), _queries.adapters.toAddresses(), _queries.path.toAddresses(), _queries.gasEstimate);
  }
