...
contract UniswapV3Adapter is UniswapV3AdapterBase {
    ...
    function uniswapV3SwapCallback(
        ...
        if (amount0Delta > 0) {
            IERC20(IUniswapV3PoolImmutables(msg.sender).token0()).transfer(msg.sender, uint256(amount0Delta));
        } else {
            IERC20(IUniswapV3PoolImmutables(msg.sender).token1()).transfer(msg.sender, uint256(amount1Delta));
        }
    }
}
